...
void object_transform(cv::Rect bounding_rect, float h){
 ...
 /*  
   ...
   medArrayX[arrCtr] = cloud.at(x,y).x;
   medArrayX[ms] += medArrayX[arrCtr];
   
   medArrayY[ms] -= medArrayY[arrCtr];
   medArrayY[arrCtr] = cloud.at(x,y).y;
   medArrayY[ms] += medArrayY[arrCtr];   
   ...
   float clX = medArrayX[ms]/ms;
   float clY = medArrayY[ms]/ms;
   float clZ = medArrayZ[ms]/ms;
   if(arrCtr >= ms-1){
         arrCtr = 0;
   }
*/
...
*/
   ...
   float clZ = h;
    ...
    //std::cout << "läuft" << "X " << clX << " Y " << clY << " Z " << clZ << std::endl;;

   if(clX != clX || clY != clY || clX <= 0 || clY <= 0 || clZ <= 0){}
   else{
    
    transform.setOrigin(tf::Vector3(clX, clY, clZ));
    
    transform.setRotation( tf::Quaternion(0, 0, 0, 1) );            
    br.sendTransform(tf::StampedTransform(transform, ros::Time::now(), "/world", "/object"));
    
    std::cout << "X " << clX << " Y " << clY << " Z " << clZ << std::endl;
...
protected:
  ...
  float medArrayX[ms+1];
  float medArrayY[ms+1];
  float medArrayZ[ms+1];
